...
<script charset="utf-8">
	$(document).ready(function() {
		...
		for (var i = 0; i < result.length; i++) {
			$('#dateposts').append('<div class="modal-dialog modal-xl" role="document">'
					+ '<div class="modal-content">' 
					+ '<div class="modal-header">' 
					+ '<h5 class="modal-title">' + formDate(result[i]["postdate"]) + '</h5></div>' + '<div class="modal-body">' + '<h5 class="input-group-prepend">@' + result[i]["usertag"] + '</h5>' + '<div class="text-center">' + '<img class="img-fluid" src="../'+result[i]["pic"]+'"></div>' + '<div class="p-5"><h5>Tags</h5>' + '<p class="form-control-plaintext text-left">' + result[i]["tags"].substring(0, result[i]["tags"].length - 1) + '</p>' + '</div></div></div></div>');
		}
